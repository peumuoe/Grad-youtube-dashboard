...
KEYWORD_NOISE_PATTERNS = (
    ...
    r"^(이런|사실|보면|말씀|그리고|대로|자체|기업들|같은|많이|어떤|겁니다|근데|가지고|있고|현재|다시|않을까|아니라|보고|가운데|명이|우리나라|기자입니다|세계|korean|하지|여러|아직|사람|정말|무슨|이건|저녁|가성비|한번|개국|역사|나오고|목표|사망한|꼽히|최근|그래|특히|때는|오히려|되는|앞서|함께|of|to|왜냐면|있고요|그거|그렇습니다)$",
)
...
def normalize_keyword_token(token: str) -> str:
    ...
    token = token.strip()
    token = KEYWORD_NORMALIZATION_MAP.get(token, token)
    if len(token) < 2:
        return ""
